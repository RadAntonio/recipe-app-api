    ...
    list=extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                'ingridients',
                OpenApiTypes.STR,
                description='Comma separated list of ingridients to filter',
            )
        ]
    )
)
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if ingridients:
            ingridient_ids = self._params_to_ints(ingridients)
            queryset = queryset.filter(ingridients__id__in=ingridient_ids)
...
class IngridientViewSet(BaseRecipeAttrViewSet):
    """Manage ingridients in teh database."""
